...
\secpage{共动体积}{$$V_{\rm comoving} = 4\pi f_{\rm sky} \int_{z_1}^{z_2} r(z)^2 \frac{dz}{H(z)}. $$}
\begin{frame}
...
\begin{frame}
  ...
  \tbox{$$V_{\rm comoving} = 4\pi f_{\rm sky} \int_{z_1}^{z_2} r(z)^2 \frac{dz}{H(z)}. $$}
  前面我们已经介绍过如何在标准$\Lambda$CDM模型里计算 $H(z)$ 以及 $r(z)$——如果你对宇宙学毫无兴趣，你已经掌握了你需要的一切，现在正是退课的绝佳时机！

  \skipline
  
 \notes{在天文学里，天区面积经常用平方度给出，一平方度对应的 $f_{\rm sky}$ 是 $\frac{\left(\frac{\pi}{180}\right)^2}{4\pi} \approx \frac{1}{41253}$.}
\end{frame}
...
\begin{frame}
 ...
 {\small 下面的近似公式就是为你量身打造，但要{\blue 注意: 公式只适用于 $0.2<\Omega_m<1$ 的平坦 $\Lambda$CDM模型里$z\lesssim 100$ 的情况}：
   \bea
   t(z) &\approx & \frac{6.5188h^{-1}\SIGyr}{\sqrt{1-\Omega_m}}\ln\frac{\sqrt{1-\Omega_m}+\sqrt{1+\Omega_m(z^3+3z^2+3z)}}{\sqrt{\Omega_m}(1+z)^{3/2}} \newl
   H(z) &\approx & H_0\sqrt{\Omega_m(1+z)^3+(1-\Omega_m)} \newl
r(z) &\approx & \frac{6243.4h^{-1}\mathrm{Mpc}}{\Omega_m^{0.395}} \times \left\{\frac{1}{(1+0.47\Omega_m)^{0.105}} \right. \newl
&& \left. - \, \frac{1}{(1+z)^{0.185}\left[1-\Omega_m + 1.47\Omega_m(1+z)^3\right]^{0.105}}\right\} \newl
d_L(z) &=& (1+z)r(z) \newl
d_A(z) &=& \frac{r(z)}{1+z} \newl
V_{\rm comoving} &=& \frac{4\pi f_{\rm sky}}{3}\left\{\left[r(z_2)\right]^3 - \left[r(z_1)\right]^3\right\}\eea
 }
\end{frame}
